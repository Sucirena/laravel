    ...
    public function store(JurnalHarianRequest $request)
    {
        ...
        try {
            ...
            if ($response->successful()) {
                ...
                // Jika hanya ada satu mood
                if (isset($data['detected_mood']) && isset($data['score'])) {
                    $emotionData = [
                        [
                            'label' => $data['detected_mood'],
                            'score' => $data['score'],
                        ]
                    ];
                } else if (isset($data['emotion_results']) && is_array($data['emotion_results'])) {
                    // Jika API suatu saat mengembalikan array emosi
                    $emotionData = [];
                    foreach ($data['emotion_results'] as $emotion) {
                        $emotionData[] = [
                            'label' => $emotion['label'],
                            'score' => $emotion['score'],
                        ];
                    ...
                    return redirect()->back()->with('error', 'Format hasil analisis emosi tidak dikenali.')->withInput();
                }
                ...
                $kontenData = [
                    ...
                    'judul' => $request->judul,
                    'isi' => $request->isi,
                    'nilai_emosi' => json_encode($emotionData),
                ];
