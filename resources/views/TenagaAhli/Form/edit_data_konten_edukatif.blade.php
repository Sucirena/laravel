...
<div class="flex flex-col w-full h-full">
    ...
    <div class=" bg-color-8 p-4 border-[1px] border-color-4 rounded-2xl">
        ...
        <form action="{{ route('kelola-konten-edukatif.update', $kontenEdukatif->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            <div class="flex flex-col items-center gap-y-5pt-10 p-10">
                ...
                @if ($kontenEdukatif->tipe == 'artikel')
                    <label class="form-control w-full mt-5">
                        ...
                        <textarea name="isi_artikel" id="editor" class="textarea textarea-bordered h-40 outline outline-1 outline-color-5 bg-color-6 rounded-lg w-full">{{ $kontenEdukatif->isi_artikel }}</textarea>
                    </label>
                    @error('isi_artikel')
                        <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                    @enderror
