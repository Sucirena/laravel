...
@section('aside')
    ...
    <div class="flex flex-col mx-auto w-full h-auto mt-9 px-12 gap-y-6">
        ...
        <a href="/daftar-aktivitas-pribadi/kustomisasi"
            class="btn flex justify-start bg-color-6 hover:bg-color-5 hover:border-color-3 text-base">
...
@section('main')
    ...
    <div id="hs-offcanvas-example"
        ...
        <div class="flex justify-between items-center py-3 px-4 border-b">
            <!-- logo -->
            <a href="{{ Auth::check() && Auth::user()->role === 'tenaga ahli' ? '/tenaga-ahli' : '/' }}"
                class="flex flex-row items-center">
                <img class="size-[1.875rem] me-0.5 md:size-12 xl:size-[3.125rem] md:me-2 xl:me-[0.938rem]"
                    src=" {{ asset('images/logo-dark-blue.svg') }} " alt="SupporT-een Logo">
        ...
        <div class="p-4">
            ...
            <div class="flex flex-col mx-auto items-center w-full h-fit gap-6">
                ...
                <!-- tombol kustomisasi aktivitas -->
                <a href="/daftar-aktivitas-pribadi/kustomisasi"
                    class="btn flex w-full justify-start bg-color-6 hover:bg-color-5 hover:border-color-3 text-base">
                    <img src="{{ asset('icons/Plus.svg') }}" alt="Plus">
                    Kustomisasi Aktivitas
                </a>
                <!-- tombol kustomisasi aktivitas -->
                ...
                @foreach ($aktivitasPribadi as $aktivitasItem)
                    <div class="flex flex-col bg-color-8 border border-color-4 rounded-3xl">
                        <div class="flex flex-col items-center p-2">
                            <img class="w-full h-auto aspect-square rounded-2xl"
                                src="{{ $aktivitasItem->aktivitasPositif->gambar ? asset( $aktivitasItem->aktivitasPositif->gambar) : asset('images/dummy.png') }}"
                                alt="Card Image">
                            <h3
                                class="text-sm md:text-lg lg:text-sm xl:text-base font-semibold md:font-bold text-color-1 text-center">
                                {{ $aktivitasItem->aktivitasPositif->nama }}
                            </h3>
                            <label class="label cursor-pointer">
                                <input type="checkbox" name="aktivitas[]" value="{{ $aktivitasItem->id }}"
    ...
    <script>
        ...
        document.getElementById('select-all-btn').addEventListener('click', function() {
            ...
            const allChecked = Array.from(checkboxes).every(checkbox => checkbox
                .checked); // Cek apakah semua sudah dicentang
